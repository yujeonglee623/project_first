...
class YouTubeCommentAnalyzer:
    ...
    def __init__(self, comments_df, videos_df=None):
        ...
        # 🔴 [수정] 'like_count' 컬럼의 존재 확인 및 정수형으로 강제 변환
    ...
    def sentiment_keywords(self):
        ...
        # 🔴 [수정 반영] 데이터 유효성 검사 추가 (ValueError 방지)
        if ordered_counts.empty:
            # 유효한 데이터가 없으면 빈 그림만 반환
...
def search_and_collect_data(keyword, max_videos, max_comments_per_video, order):
    ...
    if not api_key:
        # st.error 대신 예외 발생 (Streamlit 위젯 사용하지 않음)
        raise ConnectionError("YouTube API 키가 설정되지 않았습니다. .env 파일을 확인하세요.")
...
def get_and_cache_youtube_data(keyword, max_videos, max_comments_per_video, order):
    ...
    # search_and_collect_data의 로직을 여기에 가져옵니다. (단, 위젯은 제거)
    try:
    ...
    # 🔴 [핵심 수정] 파일 저장 로직: 캐싱 시점에 파일 저장 (딱 1회만 실행됨)
    if videos_df is not None and not videos_df.empty:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        # 1. 영상 데이터 저장: youtube_videos_YYYYMMDD_HHMMSS.csv
        ...
        # 2. 댓글 데이터 저장: youtube_comments_YYYYMMDD_HHMMSS.csv
...
def main():
    ...
    # 탭 2: 키워드 빈도 (타임스탬프 저장 적용, key 인수 추가)
    with tabs[1]:
        st.header("📊 키워드 빈도 분석")
    ...
    # 탭 3: 감성 분석 (타임스탬프 저장 적용, key 인수 추가)
    with tabs[2]:
        st.header("😊😢 감성 분석")
    ...
    # 탭 4: 시간 트렌드 (타임스탬프 저장 적용, key 인수 추가)
    with tabs[3]:
        st.header("📈 시간 트렌드 분석")
    ...
    # 탭 5: 동시출현 (타임스탬프 저장 적용, key 인수 추가)
    with tabs[4]:
    ...
    # 탭 6: 토픽 비교 (타임스탬프 저장 적용, key 인수 추가)
    with tabs[5]:
        st.header("🎬 영상별 토픽 비교")
    ...
    # 탭 7: 원본 데이터 (다운로드 파일에 타임스탬프 적용, key 인수 추가)
    with tabs[6]:
        st.header("📋 원본 데이터")
        ...
        if data_type == "댓글 데이터":
            ...
            # 🔴 [수정] 파일명 규칙 적용: youtube_comments_raw_YYYYMMDD_HHMMSS.csv
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        ...
        # 영상 데이터
            ...
            if videos_df is not None and not videos_df.empty:
                ...
                # 🔴 [수정] 파일명 규칙 적용: youtube_videos_raw_YYYYMMDD_HHMMSS.csv
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
